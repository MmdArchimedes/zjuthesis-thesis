...
从研究意义看，在理论层面，本研究有助于拓展增强现实技术在经济管理与能源政策领域的应用边界，推动区域经济研究从传统静态展示向沉浸式、交互化方向升级；在实践层面，构建的 AR 沉浸式分析系统能够为政府部门提供直观、动态、可比较的决策支持工具，帮助决策者快速把握省域数字经济与能源结构的时空差异与演变趋势，提高政策制定的精准性与时效性。
...
\medskip
\noindent\textbf{基于骨骼的手势识别方法与轻量端侧部署。}
...
\medskip
\noindent\textbf{规则式方法与学习式方法的对比与融合。}
...
本章聚焦 AR 穿戴场景下的多模态交互问题，核心贡献集中于两点：\textbf{DBEW-NN}以 57K 参数轻量网络在保持触发管线不变的前提下，将手势识别宏平均 F1 由 0.691 提升至 0.889（+28.7\%），推理时延仅 1.11\,ms；\textbf{TSTQ--Fusion}以置信度驱动的动态优先级仲裁实现手势、语音、射线事件的统一收敛，仲裁正确率 96.4\%，冲突率 3.3\%。全章按”需求$\rightarrow$手势单通道设计$\rightarrow$置信度传递$\rightarrow$跨通道融合”递进展开，为第四章可视化呈现与第五章实证应用提供低摩擦、可追溯的交互入口。
...
依据多通道用户界面理论\cite{dong2022}，本研究采用手势、射线、语音三通道互补分工：\textbf{手势}侧重短促、离散而可重复的动作，适于高频微调与小集合内的模式切换（如年份步进、指标切换）；\textbf{射线}与头显前向一致，适于与省域三角网格相交完成点选、高亮与读数等空间选择；\textbf{语音}作为辅助通道承载年份、地名与筛选条件等符号信息。三者在TSTQ--Fusion框架下统一仲裁，共同服务于AR场景下的多模态交互需求。
...
\medskip
\noindent\textbf{边沿触发与时间窗抑制。}

现有文献中的触发机制可归纳为三类：逐帧直接触发实现简单但连发严重，停留时间触发（dwell-time）通过持续保持姿态超过阈值再动作可降低误触但引入等待成本，以及基于有限状态机或置信度平滑的触发通过状态迁移提升鲁棒性但规则设计复杂\cite{wang2026ar_io_survey}\cite{duarte2020gesture}\cite{dong2022}。这些方案在通用场景有效，但面向本文”年份步进与指标切换”这类高频短指令任务时，逐帧方案的连发问题与纯停留触发的响应时延构成一对难以调和的矛盾。
...
\textbf{真实数据采集与补充微调。}在实验室可控光照条件下，招募8名参与者（均无AR设备使用经验，经5分钟预训练熟悉十一类自定义手势），佩戴Rokid Max Pro在Rokid UXR手部追踪环境下各执行5轮完整手势集合。以人工标注的起止帧和类别标签为真值。真实数据用于模型微调阶段（少量epoch）以适应实际传感器的噪声分布特征。

\textbf{数据预处理流水线。}每帧骨骼数据依次经过四个处理步骤。首先进行有效性校验，剔除追踪置信度低于阈值的无效帧。通过校验后执行腕部归一化，以腕关节（joint 0）为原点将所有关节坐标变换为腕部相对坐标，消除手部在空间中的绝对位置变化。随后进行全局缩放归一化，以所有关节坐标标准差的10倍为缩放因子将序列标准化为单位尺度，消除手型大小差异。最后以滑动窗口进行时序采样，窗口长度$T=32$帧（约533\,ms@60fps）、步长4帧，生成固定长度的输入张量$\mathbf{X}\in\mathbb{R}^{T\times J\times C}$，窗口标签采用窗口内非过渡帧的多数投票确定。训练时额外应用随机时间裁剪（$\pm5$帧）、坐标高斯噪声（$\sigma=2$\,mm）与随机镜像（概率0.5）三种数据增强。
...
在模块化与可降级层面，NN与几何规则共享统一的\texttt{GestureEvent}输出接口（字段定义见第\ref{sec:ch3_nn_deployment}节），融合层与可视化层不感知上游分类器的实现差异；当NN模型文件缺失、推理超时或连续低置信度时，系统自动回退至几何规则分类器（DBEW--Gesture），保证交互链路不被单一模块故障阻断。在置信度传递与门控协同层面，NN输出的Softmax最大概率$s^{(t)}_{\mathrm{NN}}$既作为当前帧的分类置信度写入\texttt{GestureEvent}，又参与DBEW触发管线的阈值过滤——若$s^{(t)}_{\mathrm{NN}}<\theta_g$（默认$\theta_g=0.85$），则$g_{\mathrm{NN}}^{(t)}$被强制置为NONE，从而在源头阻断低置信度输出进入边沿候选池。这一设计使NN的软概率输出与DBEW的离散门控形成级联过滤：NN先以置信度阈值过滤明显不可靠的分类，DBEW再以时序门控过滤抖动与连发，两级互补。在参数解耦与独立调优层面，触发参数$\tau$（冷却窗）、$k_{\min}$（稳定帧）和阈值$\theta_g$与NN模型参数完全解耦，触发参数可根据交互场景的实时性/稳定性需求独立调整（如走动场景加大$k_{\min}$以抑制抖动），无需重新训练或微调NN模型。第\ref{sec:ch3_nn_ablation}节的触发管线消融实验（表\ref{tab:trigger_ablation}）对此进行了定量验证。

\textbf{在线推理流程。}\label{sec:ch3_nn_inference}
...
\textbf{端侧部署与降级策略。}\label{sec:ch3_nn_deployment}

DBEW-NN的训练与部署遵循”PyTorch训练→ONNX导出→Unity Barracuda推理”的标准链路。模型经ONNX中间表示（opset 14）导出后，在Unity端通过Barracuda引擎加载执行，后端优先采用GPU Compute Shader加速并在设备不支持时自动回退至CPU优化路径。骁龙XR2平台上实测推理时延约1.1\,ms，在60FPS帧预算（16.7\,ms）中占比不足7\%，不影响渲染与SLAM追踪主循环。

NN分类器输出经DBEW触发管线稳定化后，统一封装为\texttt{GestureEvent}标准事件结构（含命令类型、时间戳、置信度与参数负载），与语音、射线事件共享TSTQ--Fusion融合层的仲裁逻辑。事件提交至融合队列后手势通道职责即告完成，后续状态更新与视图刷新均由融合层与可视化层统一处理，确保NN模块的独立升级不影响下游逻辑。

AR眼镜运行环境中存在手部追踪丢失、模型加载失败与推理超时等异常场景，系统通过分层降级策略保证交互可靠性：追踪丢失时丢弃无效帧并维护稳定计数器以防恢复后的状态不一致；模型不可用时自动回退至几何规则分类器（DBEW--Gesture）并在HUD提示降级状态；连续低置信度时暂时放宽阈值并引导用户调整手部位置。手势通道仅负责分类与触发，冲突消解交由融合层处理，这一职责分离设计保证了各通道的独立性与系统鲁棒性。
...
\textbf{识别精度对比。}\label{sec:ch3_nn_accuracy}
...
\textbf{环境鲁棒性对比。}\label{sec:ch3_nn_robustness}
...
\textbf{消融实验。}\label{sec:ch3_nn_ablation}
...
本节不写新的计量估计，而是以一次可复现的沉浸分析会话为线索，把第\ref{sec:ch3_arch_overview}节—第\ref{sec:ch3_summary}节的多模态入口、第\ref{sec:ch4_motivation}节—第\ref{sec:ch4_summary}节的可视化编排与第\ref{sec:ch5_empirical_summary}节的实证结论串成一条读者可跟随的故事线。任务设定为：在2014—2022年样本期内，围绕省域$DEL$与$ES$，完成从全国格局把握到计量证据核对，再到分区差异讨论和单省下钻阅读，全程依赖同一状态向量驱动视图刷新，避免地图直觉和表中系数脱节。

（1）进入会话与口径对齐。启动应用后，数据引擎载入与第\ref{sec:entropy_dt_fusion}节一致的指数与回归侧结构化结果（熵权初权、熵权—决策树优化权及FE主回归输出等，可与表\ref{tab:entropy_dt_compare}、表\ref{table:Baseline regression results}对照）。第四章的条件刷新规则保证，仅当用户改变年份、指标、省域或区域标签等状态变量时才触发重绘，从体验上对应先确认读什么、再动视图。

（2）全国视图。默认将全国省域Mesh置于世界空间，用户以头动环视获得东西梯度的心像。将着色通道切到$ES$并定位至2022年，可见西部省份多呈偏暖色、东部沿海多呈偏冷或偏绿色调，与图\ref{fig:4-es-coloring-2022}一致；切换到$DEL$则呈现京沪浙粤苏等相对高值集聚。沿时间轴从2014拖至2022，多数省份在$ES$通道上呈现由偏红向偏绿的渐进变化，与第\ref{sec:ch5_empirical_summary}节对样本期总体改善的叙述一致。该步骤对应第四章归一化—映射—多视图联动机制，把第\ref{sec:ch5_empirical_summary}节中倒U型讨论所需的空间分布感先交给空间编码呈现。

（3）多模态操作。用户不必回到键鼠即可完成各类交互：以射线完成省域点选与高亮，以手势完成年份步进和指标切换等离散命令（通道内已由边沿触发与时间窗抑制完成稳定化，见第\ref{sec:ch3_gesture_original}节），亦可通过语音输入年份、省份、指标等短句，映射为与手势同构的状态更新。若语音与手势在短时间窗内指向同一状态变量，融合层按第\ref{sec:ch3_fusion}节规则仲裁，避免重复写入。
...
（5）省域下钻。在射线选中浙江后，系统切换至省级视图（图\ref{fig:zhejiang}），时间轴与面板同步切换为该省口径下的$DEL$、$ES$与可用回归摘要，使讨论者能将浙江在全国梯度中的位置与该省在样本期内的轨迹放在连续注意力下完成，而不必在幻灯片与表格之间来回跳转。

（6）性能与临场体验。在Rokid Station Pro与Max Pro组合上，系统性能监测表明原型可稳定维持约30--45 FPS；图\ref{fig:ver_2figs_1cap}给出实际佩戴场景，对应边走边讲、边走边指的研讨型使用方式。整体上，这一会话把第五章的统计结论落实为第四章可核对的视图，并由第三章的自然交互降低操作摩擦，从而形成面向特定任务的完整AR可视化系统叙事。

（7）用户评测与可复现性边界。受设备与伦理流程限制，本文未展开大规模随机对照试验，但在实验室内组织了小规模任务走查（完成切换年份、切换指标、点选两省、朗读结果面板摘要等固定步骤），记录任务时延、误触次数与主观可用性评分，用于佐证交互链路可用性。在可复现性方面，计量部分可由公开统计年鉴口径与第\ref{sec:ch5_data_foundation}节变量说明复算，指数—决策树段落给出算法步骤与表\ref{tab:entropy_dt_compare}对照，AR原型依赖Unity工程版本、Rokid UXR与打包后的APK及随包CSV/JSON，读者在相同主版本栈上应能复现状态—视图一致的行为，跨机型帧率差异则属于工程边界而非统计结论部分。
