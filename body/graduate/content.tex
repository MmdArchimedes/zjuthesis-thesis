...
本文综合运用文献研究、指标综合评价、面板数据模型与工程化设计方法，整体技术路线遵循从理论到实证、从设计到实现、从分析到应用的逻辑。创新之处体现在三个方面：

\textbf{研究范式创新}：将传统经济能源实证研究与沉浸式 AR 可视化相结合，实现从二维结果展示向三维空间交互分析的跨越。

\textbf{技术融合创新}：提出 DBEW--Gesture 几何规则手势管线与 DBEW-NN 轻量神经网络增强方案（57K 参数，宏平均 F1 0.691$\rightarrow$0.889，推理时延 1.11\,ms），以及 TSTQ--Fusion 手势-语音-射线三通道融合机制（置信度驱动动态优先级仲裁，冲突率 3.3\%，仲裁正确率 96.4\%）。

\textbf{应用落地创新}：构建集实证模型、数据展示、智能问答与政策推演于一体的完整系统，基于 Unity + Rokid AR Studio 实现原型部署（30--45\,FPS），可为相关领域提供可复用的沉浸式决策支持工具。
...
综合上述三个技术维度的文献综述，可提炼以下定位本文方法的关键依据：

\textbf{手势识别维度}：现有工作在骨骼序列建模和轻量模型部署方面进展显著，但缺少在 AR 经济数据分析场景下对几何规则与轻量 NN 两种方案的精度-效率-鲁棒性三维权衡的系统对比，且端侧验证多集中于 Meta Quest 3 或 HoloLens 2，在 Rokid AR Studio 平台上的验证尚属空白。

\textbf{多模态融合维度}：75\% 的现有系统使用固定规则融合\cite{bai2025ar_gesture_speech}，缺乏对置信度信号的利用。本文 TSTQ--Fusion 通过置信度驱动动态仲裁回应了这一不足。

\textbf{AR 可视化维度}：现有方案多采用固定编码图表，缺乏声明式生成能力。本文 ChartForge 将图表生成升级为 NL$\rightarrow$规约$\rightarrow$AR 图表的 AIGC 范式。

上述三个维度在系统层面通过统一状态向量 $\mathbf{s}_t$ 实现闭环——手势、语音、射线经融合后更新 $\mathbf{s}_t$，ChartForge+SDCR--Vis 读取 $\mathbf{s}_t$ 驱动多视图协同刷新。
...
省域经济能源数据的沉浸式分析对交互系统提出三类核心需求。\textbf{空间选择}：用户需在三维省域Mesh上选中目标省份以驱动信息卡或筛选，头戴显示条件下传统桌面交互设备可达性差。\textbf{时序与指标控制}：需在2014--2022年时间轴上前后移动、在DEL/ES等多种指标间切换、刷新或重置视图，这些高频微调操作要求低时延、低误触。\textbf{符号信息输入}：需承载年份、地名与筛选条件等语义信息。
...
DBEW-NN仅替代第\ref{sec:ch3_gesture_original}节几何规则管线中的手势分类前端（伪代码第\ref{lst:gesture_pipeline}行第4步），\textbf{完全保留}其后的边沿触发、冷却窗抑制与稳定帧校验三重门控。这一“即插即用”设计具有三方面优势。

\textbf{模块化与可降级}：NN与几何规则共享统一的\texttt{GestureEvent}输出接口（字段定义见第\ref{sec:ch3_nn_deployment}节），融合层与可视化层不感知上游分类器的实现差异。当NN模型文件缺失、推理超时或连续低置信度时，系统自动回退至几何规则分类器（DBEW--Gesture），保证交互链路不被单一模块故障阻断。

\textbf{置信度传递与门控协同}：NN输出的Softmax最大概率$s^{(t)}_{\mathrm{NN}}$既作为当前帧的分类置信度写入\texttt{GestureEvent}，又参与DBEW触发管线的阈值过滤：若$s^{(t)}_{\mathrm{NN}}<\theta_g$（默认$\theta_g=0.85$），则$g_{\mathrm{NN}}^{(t)}$被强制置为NONE，从而在源头阻断低置信度输出进入边沿候选池。这一设计使NN的软概率输出与DBEW的离散门控形成\textbf{级联过滤}——NN先以置信度阈值过滤明显不可靠的分类，DBEW再以时序门控过滤抖动与连发，两级互补。

\textbf{参数解耦与独立调优}：触发参数$\tau$（冷却窗）、$k_{\min}$（稳定帧）和阈值$\theta_g$与NN模型参数完全解耦。触发参数可根据交互场景的实时性/稳定性需求独立调整（如走动场景加大$k_{\min}$以抑制抖动），无需重新训练或微调NN模型。第\ref{sec:ch3_nn_ablation}节的触发管线消融实验（表\ref{tab:trigger_ablation}）对此进行了定量验证。
...
\textbf{自注意力模块的消融}表明：移除自注意力后（CNN-only），宏平均F1从0.889降至0.884（$-$0.5个百分点），且CNN-only在少数类（如单指向左/向右）上的逐类F1波动明显大于完整方案（见表\ref{tab:gesture_accuracy}中CNN-only的逐类F1标准差），反映出Self-Attention的主要贡献在于通过全局时序上下文聚合提升逐类一致性——它使CNN提取的局部时序特征能够跨越窗口内的任意帧距离进行信息交互，减轻了CNN局部感受野在过渡帧上的类别混淆。

\textbf{窗口大小消融}表明：$T=16$时F1最低（约0.916），因为过短的窗口无法覆盖完整手势生命周期（约2\,s）；$T=32$达到最优平衡（0.889）；$T\ge48$后F1略有下降（约0.958），可能因为窗口过长引入了无关历史帧的噪声。本文选择$T=32$（约533\,ms@60FPS），在精度和缓冲区填充时延（约517\,ms）之间取得最优折中。

\textbf{数据规模消融}表明：仅使用10\%训练数据（约1名参与者）时F1已可达0.863，验证了合成数据的高质量；使用50\%数据时F1接近饱和（0.940），100\%数据仅带来约3\%的额外提升——验证了合成数据生成的多样性已基本覆盖手势分布的支撑集，继续扩大数据规模边际收益递减。
...
AR眼镜场景下的语音交互面临三重特殊约束。\textbf{（1）弱网/离线可用性}：研讨汇报、教学演示等典型使用场景中WiFi可能不稳定或不可用，纯云端ASR方案不可行，端侧离线识别为刚需。\textbf{（2）低功耗与实时性}：AR眼镜的电池与散热预算极为有限（Rokid Station Pro约200\,g含电池），语音处理须在端侧SoC上以毫秒级时延完成。\textbf{（3）多声源干扰与领域专名密集}：旁人的交谈声与环境噪声严重干扰单麦克风识别，且省域经济分析场景包含大量专有名词（省份全称、DEL/ES指标名、年份），通用ASR引擎在此类词汇上错误率显著升高。
...
ASR识别文本$\hat{s}$经以下确定性规则解析为结构化指令：

\textbf{（1）热词匹配与归一化}：将$\hat{s}$中的片段与$\mathcal{V}_{\mathrm{hot}}$进行模糊匹配（编辑距离$\leq2$），将”浙江””浙江省”等变体统一映射为标准省份名，将”二零二二”等中文数字统一转换为阿拉伯数字年份。

\textbf{（2）关键词槽位填充}：基于预定义正则模板提取指令要素——省份槽位（\texttt{\{province\}}）、年份槽位（\texttt{\{year\}}）、指标槽位（\texttt{\{indicator\}}）。例如”显示浙江省2022年DEL”经匹配后输出\texttt{\{province: 浙江省, year: 2022, indicator: DEL\}}。

\textbf{（3）拒识与降级}：若关键槽位缺失（如无法识别省份名），返回拒识标记并通过AR HUD给出轻提示，不静默执行错误操作。
