...
该机制的优势可从两类典型冲突场景中得到验证。最常见的冲突发生在手势与语音之间（占67.9\%，见表\ref{tab:fusion_stats}）：用户在语音指令未结束时无意识变动手指姿态，此时手势事件的冷却窗尚未结束（$u^{(t)}=0$），语音事件自然胜出；若两者同时有效，语音经LLM解析后的意图明确性使其评分通常高于手势的瞬时抖动。在手势与射线的冲突中，手势通道的显式控制偏置$\beta$使其在置信度接近时优先，但当射线伴随长时间凝视确认（置信度极高）时，射线可凭借更高的综合评分胜出，体现了动态优先级对上下文的自适应能力。
...
为验证TSTQ--Fusion在实际分析场景中的有效性，本文组织了小规模用户实验：6名参与者各进行约15分钟的自由分析任务，系统全程记录多通道事件与仲裁决策。实验统计表明，多通道冲突事件仅占总事件的3.3\%，仲裁正确率达96.4\%，保守拒绝率7.1\%（见表\ref{tab:fusion_stats}）。低冲突率验证了手势、语音、射线三通道在时间窗内互补分工的合理性——各通道的自然使用场景重叠较小，说明CARE模型的通道分工设计有效；高仲裁正确率则表明置信度驱动的动态优先级评分函数（式~\ref{eq:fusion_score}）能够在绝大多数冲突场景中做出符合用户预期的决策。综上，TSTQ--Fusion在静态优先级基础上引入置信度驱动的动态仲裁机制，在保持规则可解释性的同时增强了上下文适配能力，为第四章可视化联动提供了低冲突、可追溯的交互事件源。
