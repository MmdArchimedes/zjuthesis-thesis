...
数据语义层$\mathcal{D}$描述用户意图中涉及的数据字段及其语义角色：$\mathcal{D} = \big\{ (f_i,\, t_i,\, s_i,\, a_i) \big\}_{i=1}^{n}$，其中$f_i$为数据字段名（如\texttt{“DEL”}, \texttt{“province”}, \texttt{“year”}），$t_i \in \{\text{nominal}, \text{ordinal}, \text{quantitative}, \text{temporal}\}$为字段数据类型，$s_i \in \{\text{x}, \text{y}, \text{color}, \text{size}, \text{shape}, \text{facet}, \text{text}\}$为建议的视觉通道，$a_i \in \{\text{sum}, \text{mean}, \text{count}, \text{none}\}$为聚合函数。在省域经济分析场景中，数据语义层的字段词汇表与第五章指标体系严格对齐：省份、区域、年份、DEL、ES、PGDP、URBAN、INDS、TEIN等，确保了从用户说出的指标名到数据引擎中的列名再到回归表中的变量名的全程一致性。

视觉编码层$\mathcal{V} = \langle \mathcal{G},\; \mathcal{E},\; \Theta \rangle$指定图表的视觉呈现方式。$\mathcal{G} \in \Gamma$为目标图表类型，$\Gamma$覆盖柱状图、折线图、散点图、面积图、热力图、饼图、雷达图、桑基图、矩形树图、箱线图、仪表盘、漏斗图共12类常用图表；$\mathcal{E}: \mathcal{D} \to \{\text{position}, \text{color}, \text{size}, \text{shape}, \text{opacity}, \text{text}\}$为视觉编码映射函数；$\Theta = \{\theta_1, \dots, \theta_k\}$为样式参数集（配色方案、字体、标注位置等）。

交互约束层$\mathcal{I} = \big\{ (e_j,\, h_j,\, c_j) \big\}_{j=1}^{m}$描述用户对图表交互行为的期望。$e_j$为交互事件类型（点击、悬停、刷选、缩放、筛选、下钻），$h_j$为事件处理器规约，$c_j$为约束条件（如响应时间$< 200$\,ms）。该层与第三章TSTQ--Fusion的交互事件模型共享相同的类型系统，确保交互事件可在ChartForge图表和AR视图之间路由。

CIF的解析由GPT-4o结合结构化提示词完成，系统提示词定义了完整的JSON输出模式，包含字段类型约束、图表类型枚举和交互事件白名单。解析后进行三项后处理：字段验证，检查解析出的字段名是否在领域词汇表中，对误识别字段进行模糊匹配修正；类型推断补充，对缺失类型信息的字段从领域知识库中自动补充；置信度标记，解析失败的查询返回低置信度标记，由上游语音交互层通过AR HUD给出轻提示。

% =====================================================================
\section{概率图表语法（PCG）}
\label{sec:pcg}

PCG是ChartForge的理论核心，借鉴了React Reconciliation中“协调算法搜索最小变更集”的思想——正如Reconciliation在Virtual DOM树上搜索最优更新路径，PCG在图表语法树上搜索最符合用户意图的图表派生路径。其形式定义如下。
...
其中$\alpha = 0.40, \beta = 0.40, \gamma = 0.20$。

语义保真度$\Phi_{\text{sem}}$衡量图表使用的数据字段与用户意图的一致性：
...
视觉完整度$\Phi_{\text{vis}}$衡量图表视觉配置与意图的匹配度：
