...
\begin{itemize}[leftmargin=*]
    \item $f_i$为数据字段名（如\texttt{"DEL"}, \texttt{"province"}, \texttt{"year"}）；
    \item $t_i \in \{\text{nominal}, \text{ordinal}, \text{quantitative}, \text{temporal}\}$为字段数据类型；
    \item $s_i \in \{\text{x}, \text{y}, \text{color}, \text{size}, \text{shape}, \text{facet}, \text{text}\}$为建议的视觉通道；
    \item $a_i \in \{\text{sum}, \text{mean}, \text{count}, \text{none}\}$为聚合函数。
\end{itemize}
...
其中各分量含义如下：
\begin{itemize}[leftmargin=*]
    \item $\mathcal{G} \in \Gamma$为目标图表类型，$\Gamma$覆盖柱状图、折线图、散点图、面积图、热力图、饼图、雷达图、桑基图、矩形树图、箱线图、仪表盘、漏斗图共12类常用图表；
    \item $\mathcal{E}: \mathcal{D} \to \{\text{position}, \text{color}, \text{size}, \text{shape}, \text{opacity}, \text{text}\}$为视觉编码映射函数；
    \item $\Theta = \{\theta_1, \dots, \theta_k\}$为样式参数集（配色方案、字体、标注位置等）。
\end{itemize}
...
其中各分量含义如下：
\begin{itemize}[leftmargin=*]
    \item $e_j$为交互事件类型（点击、悬停、刷选、缩放、筛选、下钻）；
    \item $h_j$为事件处理器规约；
    \item $c_j$为约束条件（如响应时间$< 200$\,ms）。
\end{itemize}该层与第三章TSTQ--Fusion的交互事件模型共享相同的类型系统，确保交互事件可在ChartForge图表和AR视图之间路由。

\subsection{NL解析实现}

CIF的解析由GPT-4o结合结构化提示词完成。系统提示词定义了完整的JSON输出模式，包含字段类型约束、图表类型枚举和交互事件白名单。解析后进行三项后处理：字段验证——检查解析出的字段名是否在领域词汇表中，对误识别字段进行模糊匹配修正；类型推断补充——对缺失类型信息的字段，从领域知识库（DOMAIN\_FIELDS）中自动补充；置信度标记——解析失败的查询返回低置信度标记，由上游语音交互层通过AR HUD给出轻提示。
...
\begin{equation}
    ...
    N = \{ &
        ...
        \texttt{Scale},\, \texttt{Legend},\, \\
        & \texttt{Guide},\, \texttt{Facet},\,
...
\begin{equation}
    ...
    T = \{ &
        ...
        \texttt{AreaPath},\, \texttt{ArcGlyph},\, \\
        & \texttt{RectCell},\,
        \texttt{TextLabel},\, \texttt{TickMark},\, \texttt{GridLine},\,
        \texttt{ColorScale},\, \\
        & \texttt{SizeScale},\, \texttt{OpacityScale},\,
        \texttt{ShapeScale},\, \texttt{OrdinalAxis},\,
        \texttt{QuantitativeAxis},\, \\
        & \texttt{TemporalAxis},\, \texttt{LegendDef},\,
        \texttt{GuideLine},\, \texttt{FacetGrid},\,
        \texttt{AnnotationText},\, \\
        & \texttt{AnnotationRegion}
    \}
    \end{aligned}
\end{equation}
...
\begin{align}
    ...
    \texttt{Glyph}    &\to \texttt{BarGlyph} \mid \texttt{PointGlyph} \mid \texttt{LinePath} \nonumber\\
                      &\quad \mid \texttt{AreaPath} \mid \texttt{ArcGlyph} \mid \texttt{RectCell} \label{rule:glyph} \\
...
\begin{itemize}[leftmargin=*]
    \item 柱状图/折线图/散点图支持
          $\{\text{click}, \text{hover}, \text{brush}, \text{zoom}, \text{filter}\}$
          五类交互；
    \item 饼图/仪表盘仅支持
          $\{\text{click}, \text{hover}\}$
          两类基础交互。
\end{itemize}
若用户请求的交互类型超出图表能力范围，feasibility取0.3（而非0，因为可通过替代交互实现近似功能）。
...
\begin{definition}[参数化 Parameterize $\mathcal{P}_\theta$]
    ...
    \end{equation}
将图表模板中的自由变量替换为具体参数值。例如：
\begin{equation*}
    \mathcal{P}_{\{\text{province}=\text{浙江},\; \text{year}=2022,\; \text{metric}=\text{DEL}\}}(\text{bar\_chart})
\end{equation*}
将通用柱状图模板实例化为”2022年浙江省DEL柱状图”。
\end{definition}
...
评估采用四项指标。图表准确率（Chart Accuracy, CA）衡量图表类型、数据映射、视觉编码三者完全正确的比例，定义为CA = $\frac{1}{N}\sum_i \mathbb{1}[\text{type}_i = \text{type}^*_i \land \text{data}_i \subseteq \text{data}^*_i \land \text{enc}_i = \text{enc}^*_i]$。SVAS分数为\S\ref{sec:svas}定义的语义-视觉对齐分数，综合$\Phi_{\text{sem}}$、$\Phi_{\text{vis}}$、$\Phi_{\text{int}}$三项子指标。用户满意度（User Satisfaction, US）采用5分制Likert量表（1=非常不满意，5=非常满意）。生成延迟（Latency）指从用户NL输入到可渲染图表输出的端到端时间（秒）。
